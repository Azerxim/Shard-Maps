...
window.createMap = function () {
  // Seules les couches marquées pmIgnore: false sont éditables (entité en
  // cours d'édition), pas le spawn, les panneaux ni le contexte.
  L.PM.setOptIn(true);
  ...
  (async function () {
    ...
    const map = L.map("map", {
      ...
      maxZoom: 5,
      crs: L.CRS.Simple,
      // Requis en mode opt-in, sinon geoman n'initialise pas map.pm
      pmIgnore: false,
      maxBounds: [
    ...
    //===========================
    // EDITOR (voir editor.cartographie.js)
    highlightLayerControl();
    initCartographieEditor(map, pathname);
    // EDITOR END
    //===========================
